...
if ~isempty(start) && (length(start) ~= nvdims)
    fmt = 'length of the start index vector (%d) does not equal the number of dimensions (%d)';
    msg = sprintf ( fmt, length(start), nvdims );
    snc_error ( 'SNCTOOLS:validate_index_vectors:badStartIndex', msg );
end
if ~isempty(count) && (length(count) ~= nvdims)
    fmt = 'length of the count index vector (%d) does not equal the number of dimensions (%d)';
    msg = sprintf ( fmt, length(count), nvdims );
    error ( 'SNCTOOLS:validate_index_vectors:badCountIndex', msg );
end
if ~isempty(stride) && (length(stride) ~= nvdims)
    fmt = 'length of the stride index vector (%d) does not equal the number of dimensions (%d)';
    msg = sprintf ( fmt, length(count), nvdims );
