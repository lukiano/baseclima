...
%type_x:
%1) absolute ndegree values
%2) difference between ndegree and 20th century values
...
function comparison_plot_ndegree(scen, cvar_x, cvar_y, ndegree, year20, type_x, type_y, month, regional_masks, cluster_filename, cluster_number_mask)
    ...
    [big_data_x, modelnames_x] = create_big_data(scen, cvar_x, dirString, month, ndegree, year20, type_x);
    [big_data_y, modelnames_y] = create_big_data(scen, cvar_y, dirString, month, ndegree, year20, type_y);
    ...
    if type_x == 1
        xlabel([cvar_x ' (avg. ' num2str(ndegree) ' degree values)']);
    elseif type_x == 2
        xlabel([cvar_x ' ' num2str(ndegree) ' degrees - ' num2str(year20)]);
    elseif type_x == 3
    ...
    if type_y == 1
        ylabel([cvar_y ' (avg. year ' num2str(ndegree) ' degree values)']);
    elseif type_y == 2
        ylabel([cvar_y ' ' num2str(ndegree) ' degrees - ' num2str(year20)]);
    elseif type_y == 3
